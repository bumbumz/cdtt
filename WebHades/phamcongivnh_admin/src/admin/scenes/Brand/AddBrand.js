...
function AddBrand() {
    const [brand, setBrand] = useState({
        ...
        slug: '',
        description: '',
        image:null, // Sử dụng null cho file hình ảnh
        sort_order: '',
    });
    ...
    const handleAddBrand = async (e) => {
        ...
        formData.append('image', brand.image); // Gửi hình ảnh
        formData.append('sort_order', brand.sort_order);

        try {
            await BrandService.add(formData); // Gọi API thêm brand
            navigate('/admin/brand'); // Điều hướng về trang danh sách thương hiệu sau khi thêm thành công
        } catch (error) {
