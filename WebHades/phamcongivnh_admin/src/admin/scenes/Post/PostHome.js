...
function PostHome() {
    ...
    useEffect(() => {
        const fetchBanners = async () => {
            try {
                const response = await PostHomeService.getList();
                if (response && response.postshome) {
                    setBanners(response.postshome); // Lưu danh sách banner
                } else {
                    console.error("Unexpected response format:", response);
                }
                setLoading(false);
            } catch (err) {
                console.error("Error fetching postshome:", err);
                setError(err);
                setLoading(false);
            }
        };
    ...
    const handleStatusChange = async (id, currentStatus) => {
        ...
        try {
            await PostHomeService.editstatus(id, { status: newStatus }); // Gọi API để cập nhật trạng thái
    ...
    //         try {
    //             await PostHomeService.delete(id);
            ...
            <table className="min-w-full border-collapse border border-gray-400">
                <thead>
                    <tr className="bg-gray-800 text-white">
                        ...
                        <th className="border border-gray-400 p-2">Tên bài viết</th>
                ...
                <tbody>
                    {banners.map((banner) => (
                        <tr key={banner.id} className="hover:bg-gray-700 text-white">
                            ...
                            <td className="border border-gray-400 p-2">{banner.conten}</td>
                            <td className="border-t border-gray-400 p-2 flex space-x-2  items-start justify-center ">
                                <Link
                                    to={`/admin/posthome/edit/${banner.id}`}
